...
[dataRecebimentoEl, meioPagamentoEl, observacaoEl, descontoValorEl].forEach(el => el.addEventListener('input', onFormChanged));
...
function calcularDesconto() {
  ...
  const d = toNumber(descontoValorEl.value);
  if (d <= 0) return 0;
  return Math.min(d, subtotal); // desconto nunca deixa o total negativo
}
...
function onFormChanged(skipRowsRerender) {
  document.getElementById('subtotalFormulario').textContent = formatBRL(calcularSubtotal());
...
function renderReceiptPreview(targetEl, d, opts) {
  ...
  targetEl.innerHTML = `
    ...
    <div class="r-body">
      ...
      ${d.desconto > 0 ? `
      <div class="r-subtotal-row"><span>Subtotal</span><span>${formatBRL(d.subtotal)}</span></div>
      <div class="r-subtotal-row is-desconto"><span>Desconto</span><span>- ${formatBRL(d.desconto)}</span></div>
      ` : ''}
...
function computeContentEndY(d, comprovanteLines, obsLines) {
  ...
  cy += 24;
  if (d.desconto > 0) cy += 18 * 2; // linhas de subtotal e desconto
...
async function drawReceiptToCanvas(d, opts) {
  ...
  // Renderiza em resolução mais alta que o tamanho "lógico" (W x H), como
  // as telas retina/iPhone fazem — sem isso, o PNG sai borrado e ilegível.
  ...
  ctx.scale(SCALE, SCALE); // a partir daqui, todo o código de desenho continua usando as coordenadas W x H normais
  ...
  // subtotal / desconto (só aparecem quando há desconto aplicado)
  if (d.desconto > 0) {
    ctx.font = '12px -apple-system, Segoe UI, Arial, sans-serif';
    ctx.fillStyle = colors.soft;
    ctx.fillText('Subtotal', PAD + 16, cy + 14);
    ctx.textAlign = 'right';
    ctx.fillText(formatBRL(d.subtotal), W - PAD - 16, cy + 14);
    ctx.textAlign = 'left';
    cy += 18;

    ctx.fillStyle = '#b3432f';
    ctx.fillText('Desconto', PAD + 16, cy + 14);
    ctx.textAlign = 'right';
    ctx.fillText('- ' + formatBRL(d.desconto), W - PAD - 16, cy + 14);
    ctx.textAlign = 'left';
    cy += 18;
  }
...
function abrirReciboDoHistorico(id) {
  ...
  descontoValorEl.value = r.desconto || 0;
  previewOverrideNumero = r.numero;
  renderItemsRows();
  document.getElementById('subtotalFormulario').textContent = formatBRL(r.subtotal ?? r.total);
